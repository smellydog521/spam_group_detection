...
def get_sprites(reviewer, url, writer):
    # to get item id
    # url_list = url.split(r'/')
    # item_id, a, b = url_list[-1].split(r'_')
    ...
    # bs
    # file_obj = open('5375281_7c5554b2-c5a5-47c6-84a7-9c423ae52343_1.html', 'r', encoding='utf-8')
    ...
    # all_comment_items = soup.find('div', id="comment-0")  # 第1页
    # print(all_comment_items)

    # to find any <a href='//club...' target='_blank'>
    # check: findAll('span', {'class': {'green', 'red'}})
    ...
    # comment = detail.find('div', id='comment')
    # mc = comment.find('div', class_='mc')
    # j_comments_list = mc.find('div', class_='J-comments-list comments-list ETab')
    # tab_con = j_comments_list.find('div', class_='tab-con')
    # # comments = tab_con.find_all('div', {'class': False})
    # comment_0 = tab_con.find('div', id='comment-0')
    # print(f'comment_0: {comment_0}')
    # # print(f'comments: {comments}')
    # # comment_items = comments[0].find_all('div', class_='comment-item')
    # comment_items = comment_0.find_all('div', class_='comment-item')
    # print(f'comment_items: {comment_items}')
    # comment_column = comment_items[0].find('div', class_='comment-column J-comment-column')
    # comment_message = comment_column.find('div', class_='comment-message')
    # comment_op = comment_message.find('div', class_='comment-op')
    # sprite = comment_op.find('a', {'target': '_blank'})
    # print('url: {}'.format(sprite['href']))
    ...
    for each_comment in comments:
        ...
        time = tc.find('span', class_='time').text
        writer.writerow([reviewer, follower, content, time])
...
def run(item):
    ...
    suffix = r'_1.html'
    csv_path = item + '_sprites.csv'
    sprite_csv = open(csv_path, 'w', encoding="utf-8", newline='')
    writer = csv.writer(sprite_csv)
    writer.writerow(['reviewer', "follower", "content", 'time'])  # 写入标题
    ...
    for index, row in data.iterrows():
        ...
        print('-' * 50 + 'New URL' + '-' * 50)
        print(url)
        get_sprites(reviewer, url, writer)
    sprite_csv.close()
